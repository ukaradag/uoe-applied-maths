...
plot.tps <- function(tps_object, plot_type = "persp", ...) {
  ...
  for (i in seq_len(nrow(grid_points))) {
    E_grid[i, ] <- sapply(seq_len(nrow(xk)), function(j) eta(norm(as.matrix(grid_points[i, ]) - xk[j, ], '2')))
  }

  # Now try the multiplication
  tryCatch({
    predictions <- E_grid %*% alpha + cbind(1, grid_points) %*% delta
  }, warning = function(w) {
  ...
  # Create the plot
  if (plot_type == "persp") {
    persp(x_seq, y_seq, predictions_matrix, ...)
  } else if (plot_type == "contour") {
    contour(x_seq, y_seq, predictions_matrix, ...)
  }
}

# To call this method assuming `res_tps` is your tps object:
# Make sure to set the class attribute to 'tps' if it's not already set
class(res_tps) <- 'tps'
